...
class ResPartner(models.Model):
    ...
    def _get_latest_level_id_without_lit(self):
        company = self.env.user.company_id
        for partner in self:
            amls = partner.unreconciled_aml_ids
            latest_date = False
    ...
    latest_followup_date = fields.Date(
        compute='_get_latest_followup_date', string="Latest Follow-up Date", compute_sudo=True,
        help="Latest date that the follow-up level of the partner was changed"
    )
    latest_followup_level_id = fields.Many2one(
        'followup.line', compute='_get_latest_level_id', compute_sudo=True,
    ...
    latest_followup_level_id_without_lit = fields.Many2one(
        'followup.line', compute='_get_latest_level_id_without_lit', store=True, compute_sudo=True,
