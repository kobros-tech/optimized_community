...
class StockQuant(models.Model):
    ...
    def _onchange_product_id_auto_fill_lot(self):
        if self.product_id and self.product_id.tracking in ['serial', 'lot'] and not self.lot_id:
            lot_numbers = self.env['stock.lot']
            lot_serial_no_id = self.create_serial_no_stock()
            existing_line = lot_numbers.search([('name', '=', lot_serial_no_id)])

            while existing_line:
                lot_serial_no_id = self.create_serial_no_stock()
                existing_line = lot_numbers.search([('name', '=', lot_serial_no_id)])

            self.lot_id = lot_numbers.create({
                'name': lot_serial_no_id,
